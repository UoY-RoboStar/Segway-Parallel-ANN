...
"HiddenLayer(l :: int, s :: int, inpSize :: int) = (( \<lbrakk> \<lbrace> layerRes.(l-1) \<rbrace> \<rbrakk> i \<in> {1..s} \<bullet> Node(l,i,inpSize)))" | 

"HiddenLayers = HiddenLayer(1,1,2)" | 

"OutputLayer = ( \<lbrakk> \<lbrace> layerRes.(#inRanges-1) \<rbrace> \<rbrakk> i \<in> {1..1} \<bullet> Node(l,i,1))" | 

"ANN = ((HiddenLayers
  \<lbrakk> | \<lbrace> layerRes.(#inRanges-1) \<rbrace> | \<rbrakk> 
  OutputLayer);; ANN)" | 
